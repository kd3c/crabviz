// Full CallGraph interaction logic ported from webview-ui/src/graph/CallGraph.ts
// Kept verbatim with minimal adjustments (global export) to restore UI parity.
...
export class CallGraph {
  ...
  public onSelectElem = (elem: SVGElement | null) => {
    ...
    const classes = elem.classList;
    if (classes.contains('node')) this.onSelectNode(elem);
    else if (classes.contains('cell')) this.onSelectCell(elem);
    else if (classes.contains('edge')) this.onSelectEdge(elem);
    else if (classes.contains('cluster-label')) this.onSelctCluster(elem);
  };
  ...
  onSelectCell(cell: SVGElement) {
    const id = cell.id;
    if (this.focus) this.onSelectCellInFocusMode(id);
    else {
      const cellIds = new Set([cell.id]);
      cell.querySelectorAll('.cell').forEach((c) => { cellIds.add((c as any).id); });
      this.highlightEdges((edge) => [
  ...
  onSelctCluster(clusterLabel: SVGElement) {
    ...
    this.nodes.forEach((node) => { if (this.rectContains(rect, node.getBoundingClientRect())) selected.add(node.id); });
    this.highlightEdges((edge) => [
  ...
  highlightEdges(judge: (edge: SVGGElement) => [boolean, boolean]) {
    ...
    for (const edge of this.edges) {
      ...
      if (incoming) edge.classList.add('incoming');
      if (outgoing) edge.classList.add('outgoing');
      if (!(incoming || outgoing)) fadedGroup.appendChild(edge);
    }
  }

  fadeOutNodes(kept?: Set<string>) {
    if (!kept) kept = new Set();
    const fadedGroup = this.svg.getElementById('faded-group')!;
    for (const edge of this.edges) {
      if (edge.parentElement === fadedGroup) continue;
      kept.add(this.getNodeId(edge.dataset.from!)).add(this.getNodeId(edge.dataset.to!));
    }
    const clusters = new Set(this.clusters);
    for (const node of this.nodes) {
      if (!kept.has(node.id)) { fadedGroup.appendChild(node); continue; }
      const rect = node.getBoundingClientRect();
      for (const cluster of clusters) {
        if (this.rectContains(cluster.getBoundingClientRect(), rect)) clusters.delete(cluster);
      }
    }
    fadedGroup.append(...clusters);
  }

  getNodeId(id: string): string { return id.substring(0, id.indexOf(':')); }
  rectContains(r1: DOMRect, r2: DOMRect): boolean { return r1.left < r2.left && r1.right > r2.right && r1.bottom > r2.bottom && r1.top < r2.top; }
}

// Attach to global for inline runtime init
(globalThis as any).CallGraph = CallGraph;
